...
We see that even though the random effects model is recommended for systematic reviews, it is only used in 32% of the meta-analyses in our dataset. 
...
We see that the estimates for $\tau^2$ can vary considerably between each estimation method: In CD007944, the Hunter-Schmidt estimate for $\tau^2$ is about .55  whereas the Hedges estimate is almost 1.1. While there is no obvious pattern, it does appear that the Sidik-Jonkman estimate usually returns the highest $\tau^2$.
...
We see that the Sidik-Jonkman estimator on average returns higher estimates for $\tau^2$. While the other estimators are modal near zero, the mode of the Sidik-Jonkman estimator is about .11.
...
We see that the Hedges estimator on average is estimated to have a higher standard error. 
...
Since we see that there is some discrepancy between the $\tau^2$ estimates, we also examine their effect on the meta-analysis summary estimate. Here we plot the meta analysis summary estimates for the same 60 random studies that we looked at above: 
